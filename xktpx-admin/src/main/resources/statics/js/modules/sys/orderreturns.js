$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/orderreturns/list',
        datatype: "json",
        colModel: [			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.orderReturns.id == null ? "sys/orderreturns/save" : "sys/orderreturns/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "sys/orderreturns/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/orderreturns/info/"+id, function(r){
