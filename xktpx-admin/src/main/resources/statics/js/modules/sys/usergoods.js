$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/usergoods/list',
        datatype: "json",
        colModel: [			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.userGoods.id == null ? "sys/usergoods/save" : "sys/usergoods/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "sys/usergoods/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/usergoods/info/"+id, function(r){
