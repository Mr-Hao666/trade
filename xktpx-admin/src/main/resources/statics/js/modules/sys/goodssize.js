$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/goodssize/list',
        datatype: "json",
        colModel: [			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.goodsSize.id == null ? "sys/goodssize/save" : "sys/goodssize/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "sys/goodssize/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/goodssize/info/"+id, function(r){
